...
If you use animation library in your custom wrapper component, then during step change Formik logs warning "A component is changing an uncontrolled input to be controlled". Tested with `react-spring` and `framer-motion`. Spent days debugging the issue but still not sure why it happens.
